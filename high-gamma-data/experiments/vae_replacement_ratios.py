...
METHOD_ORDER = {
    "vae_replace_0pct_real": 0,
    "vae_replace_25pct_real": 25,
    "vae_replace_50pct_real": 50,
    "vae_replace_75pct_real": 75,
    "vae_replace_100pct_real": 100,
}

RATIO_PROTOCOL_ID = (
    "vae_reconstruction_fixed_size_ratios_90_10_seed42"
)
...
RATIO_RAW_RESULTS = (
    RATIO_DIRECTORY
    / "vae_replacement_ratio_runs.csv"
)

RATIO_PARTICIPANT_SUMMARY = (
    RATIO_DIRECTORY
    / "vae_replacement_ratio_participant_summary.csv"
)

RATIO_METHOD_SUMMARY = (
    RATIO_DIRECTORY
    / "vae_replacement_ratio_method_summary.csv"
)

COMPLETION_MARKER = (
    CONFIG.manifest_directory
    / "vae_replacement_ratios_completed.txt"
)
...
def load_reconstruction(
    ...
    if not path.exists():
        raise FileNotFoundError(
            f"Missing final VAE reconstruction: {path}"
        )
...
def create_combined_summary() -> None:
    ...
    print(
        "FIXED-SIZE VAE REPLACEMENT SUMMARY",
        flush=True,
    )
    print(
        "=" * 80,
